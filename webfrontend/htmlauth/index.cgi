...
if ( !$query{'guek'} )   { if ( param('guek')  ) { $guek = quotemeta(param('guek'));         } 
else { $guek = $guek;  } } else { $guek = quotemeta($query{'guek'});   }
...
# ---------------------------------------
if (param('savedata')) {
	$conf = new Config::Simple("$home/config/plugins/$psubfolder/smartmeterstyria.cfg");

	if ($enabled ne 1) { $enabled = 0 }

	$conf->param('SMARTMETERSTYRIA.GUEK', unquotemeta($guek));
	$conf->param('SMARTMETERSTYRIA.MINISERVER', unquotemeta("MINISERVER$miniserver"));
	$conf->param('SMARTMETERSTYRIA.UDPPORT', unquotemeta($msudpport));
	$conf->param('SMARTMETERSTYRIA.ENABLED', unquotemeta($enabled));
...
# ---------------------------------------
$conf = new Config::Simple("$home/config/plugins/$psubfolder/smartmeterstyria.cfg");
$guek = encode_entities($conf->param('SMARTMETERSTYRIA.GUEK'));
$miniserver = encode_entities($conf->param('SMARTMETERSTYRIA.MINISERVER'));
$msudpport = encode_entities($conf->param('SMARTMETERSTYRIA.UDPPORT'));
$enabled = encode_entities($conf->param('SMARTMETERSTYRIA.ENABLED'));
...
# Title
$template_title = $phrase->param("TXT0000") . ": SmartMeterStyria";
